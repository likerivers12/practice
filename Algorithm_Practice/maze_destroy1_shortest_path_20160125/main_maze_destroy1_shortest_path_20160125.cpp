...
// VISIT 표시 구분하고 movable check 개선 후 성공
...
#define D_NOPATH 3
#define D_CUR	8
#define D_VISIT_DESTROYABLE 4
#define D_VISIT_NOTDESTROYABLE 5
...
bool is_movable(int next_r, int next_c, bool destroyable)
{
	...
	if (map[next_r][next_c] == D_EMPTY) {
		...
		return true;
	} else if (map[next_r][next_c] == D_VISIT_NOTDESTROYABLE && destroyable ) {
		return true;
	} else {
		return false;
	}
	...
	while ( !is_emptyQ() ) {
		...
		if ( is_movable(next_r, next_c, destroyable) ) {
			...
			if (map[cur_r][cur_c] == D_WALL) {
				...
				map[cur_r][cur_c] = D_VISIT_DESTROYABLE;
			} else if (!destroyable) {
				map[cur_r][cur_c] = D_VISIT_NOTDESTROYABLE;
			} else {
